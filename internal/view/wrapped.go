...
func Wrapped(w *service.Wrapped, loc *time.Location) string {
	if w.Added == 0 {
	...
	if w.First != nil {
		fmt.Fprintf(&sb, "最初の一曲: %s（%s）\n", w.First.Video.Title, w.First.Item.AddedAt.In(loc).Format("01/02"))
		fmt.Fprintf(&sb, "最後の一曲: %s（%s）\n", w.Last.Video.Title, w.Last.Item.AddedAt.In(loc).Format("01/02"))
	}

	if len(w.TopViewed) > 0 {
		sb.WriteString("\n**この年に追加した曲の再生数ランキング**\n")
		for i, v := range w.TopViewed {
			fmt.Fprintf(&sb, "%d. %s（%s 回）\n", i+1, v.Title, Number(v.Views))
		}
	}
