...
type LibraryService struct {
	...
	// Serializes Sync and RefreshVideos, which both write videos
	mu    sync.Mutex
...
func (s *LibraryService) syncPlaylist(meta *repository.PlaylistMeta) (*PlaylistSnapshot, error) {
	...
	changed := map[string]*domain.YouTubeVideo{}
	var needDetails []string
	for _, f := range fetched {
		video, ok := videos[f.VideoID]
		if !ok {
			video = &domain.YouTubeVideo{YoutubeID: f.VideoID}
	...
	if len(needDetails) > 0 {
		details, err := s.youtube.FetchVideos(unique(needDetails), true)
		if err != nil {
			return nil, err
		}
	...
	var toSave []*domain.YouTubeVideo
...
func (s *LibraryService) RefreshVideos() error {
	...
	fetched, err := s.youtube.FetchVideos(targets, false)
	if err != nil {
		return err
	}
	...
	for _, f := range fetched {
		...
		// Keep the icon, which is not fetched here
		f.ChannelIcon = video.ChannelIcon
...
func mergeDetails(dst *domain.YouTubeVideo, src *domain.YouTubeVideo) {
	dst.Title = src.Title
	dst.ChannelID = src.ChannelID
	dst.ChannelName = src.ChannelName
	dst.ChannelIcon = src.ChannelIcon
